...
//			if(todd->RSTO == todd->intrvlBeh) {
//				knob* rstocpy = calloc(1, sizeof(knob));
//				*rstocpy = *todd->intrvlBeh;
//				crik->interval = rstocpy;
//			} else
...
// Note     : for speed up purpose, this function is rather long
//*****************************************************************************
int jump_stage_2_fit_hlix(config* seq, global* crik, local* toddP,
		int8_t recurRoute) {
	jst2Count++;
...
//						dispLL(seq,crik,todd,toddP);
						...
						if (hlixCursr) {
							...
							if ((hlixCursr->lvlOfRecur
									== intrvlMsgr->lvlOfRecur + 1) && // || REPLACE OLD HELIX BY NEW ONE
								...
								if (!cmpntCursr->bundleFlag) {
									...
									for (q = 0; q < seq->maxNumMismatch; q++) {
										if (cmpntCursr->mismatchFlag[q][0]
												== -1) {
											maxMismatches = 0;
										} else {
											numMismatches++;
								...
								if (hlixCursr && cmpntCursr
										&& hlixCursr->jumpTreeNext && (hlixCursr->jumpTreeNext->opnBrsInnIndx
												- cmpntCursr->opnBrsOutIndx
												== -1) && // || l*p + k rule in the case of add new hlix w/o removing old one
										(hlixCursr->jumpTreeNext->closeBrsInnIndx
												- cmpntCursr->closeBrsOutIndx
												== 1)) {
									int q;
									if (!hlixCursr->jumpTreeNext->bundleFlag) {
										for (q = 0; q < seq->maxNumMismatch;
												q++) {
											if (hlixCursr->jumpTreeNext->mismatchFlag[q][0]
													!= -1) {
												numMismatches++;
											}
										}
									...
									if (seq->maxNumMismatch > 0) {
										if (((hlixCursr->jumpTreeNext->opnBrsInnIndx
												- hlixCursr->jumpTreeNext->opnBrsOutIndx
												!= (seq->minLenOfHlix - 1))
												&& ((hlixCursr->jumpTreeNext->mismatchFlag[seq->maxNumMismatch
														- 1][0]
														!= hlixCursr->jumpTreeNext->opnBrsInnIndx
														|| maxMismatches)
														|| ((cmpntCursr->opnBrsInnIndx
																...
																>= (seq->minLenOfHlix
																		* 3)
																&& !cmpntCursr->bundleFlag
																&& !hlixCursr->jumpTreeNext->bundleFlag))
												&& (!cmpntCursr->bundleFlag
														&& (cmpntCursr->mismatchFlag[0][0]
																!= cmpntCursr->opnBrsOutIndx
																|| cmpntCursr->opnBrsInnIndx
																		- cmpntCursr->opnBrsOutIndx
																		+ 1
								...
								if (seq->numCovari) { // || || update constraint flags (currently, only V1 and covariance are included)
									...
									for (j = 0; j < flagUB; j++) {      // || ||
										if (!cmpntCursr->mustPairFlag[j]
												&& hlixCursr->mustPairFlag[j]) { // || || replace old hlix will lose a covariance pair or v1 pairing in helix
										...
										// || ||
										tempInt[j] =
												(cmpntCursr->mustPairFlag[j]
														|| crik->struMustPairFlag[j]); // || || if there's no issue of losing covariance pair, then the new flag set should contain all flags
									} // end for                                                                       // || ||
									  // || ||
								  ...
								  // ||
								cmpntCursr->jumpTreeNext =
										hlixCursr->jumpTreeNext; // || || unhook the reference to old helix, and hook reference to the new helix. erase the record of the old helix, so that it may be used again somewhere else
								...
								hlixCursr->lvlOfRecur = intrvlMsgr->lvlOfRecur
										+ 1;                            // || ||
								hlixCursr->hlixBranchngIndx1 =
										intrvlMsgr->hlixBranchngIndx1;  // || ||
																		// || \/
								int rstoFlag = 0;								// ||
								if (todd->RSTO
										&& !is_there_concern_on_ring_formation_of_linked_list(
												seq, crik, todd)) {
									disp(seq,DISP_ALL,"            special On Q %d-%d\n",todd->RSTO->opnBrsInnIndx, todd->RSTO->closeBrsInnIndx);
									crik->interval = todd->RSTO;
									rstoFlag = 1;
...
//		}
								...
								/*
								 // DEBUGGING!!
								 if (hlixCursr->jumpTreeNext &&
								 hlixCursr->opnBrsOutIndx == 10 && hlixCursr->opnBrsInnIndx == 14 && hlixCursr->closeBrsInnIndx == 28
								 && hlixCursr->closeBrsOutIndx == 32 && hlixCursr->jumpTreeNext->opnBrsOutIndx == 15 &&
								 hlixCursr->jumpTreeNext->opnBrsInnIndx == 19 && hlixCursr->jumpTreeNext->closeBrsInnIndx == 23 &&
								 hlixCursr->jumpTreeNext->closeBrsOutIndx == 27) {
								 printf("So... it has come to this.\n");
								 } else if (hlixCursr->jumpTreeNext &&
								 hlixCursr->opnBrsOutIndx == 15 && hlixCursr->opnBrsInnIndx == 19 && hlixCursr->closeBrsInnIndx == 23
								 && hlixCursr->closeBrsOutIndx == 27 && hlixCursr->jumpTreeNext->opnBrsOutIndx == 10 &&
								 hlixCursr->jumpTreeNext->opnBrsInnIndx == 14 && hlixCursr->jumpTreeNext->closeBrsInnIndx == 28 &&
								 hlixCursr->jumpTreeNext->closeBrsOutIndx == 32) {
								 printf("So... it has come this this.\n");
								 }
								 */
								...
								/*
								 // DEBUGGING
								 if (hlixCursr->opnBrsOutIndx == 15 && hlixCursr->opnBrsInnIndx == 19 && hlixCursr->closeBrsInnIndx == 23 && hlixCursr->closeBrsOutIndx == 27
								 && hlixCursr->jumpTreeNext->opnBrsOutIndx == 11 && hlixCursr->jumpTreeNext->opnBrsInnIndx == 14 &&
								 hlixCursr->jumpTreeNext->closeBrsInnIndx == 28 && hlixCursr->jumpTreeNext->closeBrsOutIndx == 31) {
								 printf("maxMismatches=%d ", maxMismatches);
								 int length2 = hlixCursr->opnBrsInnIndx - hlixCursr->jumpTreeNext->opnBrsOutIndx + 1;
								 printf("length=%d\n", length2);
								 }
								 */
								...
								if ((hlixCursr->jumpTreeNext->opnBrsInnIndx
										...
										&& ((hlixCursr->jumpTreeNext->mismatchFlag[seq->maxNumMismatch
												...
												|| ((hlixCursr->opnBrsInnIndx
														...
														>= (seq->minLenOfHlix
																* 3)
														&& !hlixCursr->bundleFlag
														&& !hlixCursr->jumpTreeNext->bundleFlag))
										&& (!hlixCursr->bundleFlag
												&& (hlixCursr->mismatchFlag[0][0]
...
//		    }  // end inner 2 if                           // ||
									...
									/*
									 if (i == 0) { // HACK in order to allow type 1 components to fit
									 crik->hackCounter++;
									 if (crik->hackCounter == 5)
									 break;
									 }
									 */
...
int is_there_larger_dumi_2_replace_curr_candidate_cmpnt_with_concern_on_cmpnt_insert_behind(
		global* crik, knob* cmpntCursr, int16_t iCmpntTyp,
		int16_t cmpntOcupidID) {
	int16_t j;
...
int is_there_larger_dumi_2_replace_curr_candidate_cmpnt_wo_concern_on_cmpnt_insert_behind(
		config* seq, global* crik, knob* cmpntCursr, int16_t bundleSpan,
		int16_t iCmpntTyp) {
	int16_t jUB =
			(iCmpntTyp + bundleSpan < seq->strLen) ?
					(iCmpntTyp + bundleSpan) : seq->strLen; // no worry, no component can insert behind, so just go ahead and expand 'j' to its allowed limit, which is the bundle span
	int16_t j;
...
int remove_intrvl(config* seq, global* crik, local* toddP, int8_t recurRoute) {
	...
	if (is_intrvl_2b_rsto(crik, toddP, recurRoute)
			&& !is_there_concern_on_ring_formation_of_linked_list(seq, crik,
					toddP)) {
/*		if(toddP->RSTO == toddP->intrvlBeh) {
			knob* behCpy = calloc(1, sizeof(knob));
			*behCpy = *toddP->intrvlBeh;
			toddP->intrvlBeh = behCpy;
		}
*/		
...
// Display  : none
//*****************************************************************************
int search_4_largest_rep_of_this_bundle_group(global* crik, local* todd,
		int16_t cmpntOcupidID) {
	int16_t i = cmpntOcupidID;

	if (todd->cmpntLLCursr->closeBrsOutIndx
			>= crik->cmpntListOcupidTyp[crik->numCmpntTypOcupid - 1]) { // when if is true, it means there's no other component available to insert on the right of this current component referred by 'todd->cmpntLLCursr'
		...
		while (todd->cmpntLLCursr->closeBrsOutIndx
				>= crik->cmpntListOcupidTyp[i])
			i++; // search for the component type which is closest to trailing edge (closeBrsOutIndx) of current compoent, but larger
		while ((todd->cmpntLLCursr->cmpntListNext)
				&& // make sure there's larger component available at cmpntListNext
				(todd->cmpntLLCursr->cmpntListNext->closeBrsOutIndx
						< crik->cmpntListOcupidTyp[i])
				&& (crik->eden[todd->cmpntLLCursr->cmpntListNext->opnBrsOutIndx][todd->cmpntLLCursr->cmpntListNext->closeBrsOutIndx].dumiNode)) // make sure this larger component isn't too large to exceed bundle list upper bound
			todd->cmpntLLCursr = todd->cmpntLLCursr->cmpntListNext; // if the above case is true, there's no need to print out each of the component indvidually, but can just pick the longest one as representative under the same 'bundle' in a sense
	}  // end if
...
// Display  : none
//*****************************************************************************
int take_cmpnt_list_normal_path(config* seq, global* crik, local* todd,
		int16_t hlixBranchngIndx1) {
