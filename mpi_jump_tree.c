...
int is_work_needed() {
  ...
  if(flag) {
    int message[4];
    MPI_Recv(message, 4, MPI_INT, ms.MPI_SOURCE, MPI_REQUEST_WORK, MPI_COMM_WORLD, &ms);

    // should I send work?
    if(mpi_stage == MY_MPI_STAGE_2 && mpi_workleft) {
      return message[0];
    }
...
void prepare_to_work(MPI_Status ms, config* seq, global* crik, local* todd) {
  ...
  unpack_structure(crik, message, num_elements);
...
void get_work(int amount, config* seq, global* crik, local* todd) {
  ...
  while(1) {
    ...
    if(ms.MPI_TAG == MPI_WORK) {
      ...
      MPI_Recv(message, 4, MPI_INT, mpi_from, MPI_REQUEST_WORK, MPI_COMM_WORLD, &ms);
...
int pack_structure(global* crik, int* msg) {
  int count = 0;
...
void unpack_intervalKnob(knob* intrvl, int* msg) {
  knob* cur = intrvl;
...
f
  int* message;
  int cmpnts = pack_structure(crik, message);
